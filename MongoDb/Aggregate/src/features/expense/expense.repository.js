...
class ExpenseRepository {
    ...
    async aggregateTotalRevenue() {
        const db = getDB();
        const expenses = await db.collection(this.collectionName).aggregate([{ $group: { _id: "$title", totalRevenue: { $sum: "$amount" } } }]).toArray();
        return expenses;
    }
    ...
    async groupExpensesByTags() {
        const db = getDB();
        const expenses = await db.collection(this.collectionName).aggregate([{ $group: { _id: "$tags", expenses: { $push: { _id: "$_id", title: "$title", amount: "$amount", date: "$date", isRecurring: "$isRecurring", tags: "$tags" } } } }]).toArray();
        return expenses
    }
    ...
    async groupAndCalculateAvgByRecurring() {
        const db = getDB();
        const expenses = await db.collection(this.collectionName).aggregate([{ $group: { _id: "$isRecurring", avgAmount: { $avg: "$amount" } } }]).toArray();
        return expenses;
    }
}
