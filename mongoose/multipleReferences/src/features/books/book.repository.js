...
export default class BookRepository {
    ...
    async addReviewToTarget(targetId, target, text, rating) {
        if (target != 'Author' && target != 'Book') {
            throw new Error("Specified target is not matching")
        }
        if (target == "Book") {
            const book = await booksModel.findById(targetId).exec();
            if (!book) {
                throw new Error("The specified target entity Book was not found.")
            }
            ...
            const author = await authorsModel.findById(targetId).exec();
            if (!author) {
                throw new Error("The specified target entity Author was not found.")
            }
            const review = new reviewModel({
                text: text,
                target: target,
